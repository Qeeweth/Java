...
public class Client {
    public static void main(String[] args) {
        try {
            Socket socket = new Socket("127.0.0.1", 8188); // Создаем сокет для подключения к серверу
            System.out.println("Успешно подключен");
            DataOutputStream out = new DataOutputStream(socket.getOutputStream());
            DataInputStream in = new DataInputStream(socket.getInputStream());
            Thread thread = new Thread(new Runnable() { // Создаем поток для приема сообщения от сервера
                @Override
                public void run() {
            ...
            String request = null;
            while (true) {
                request = scanner.nextLine(); // Ждем сообщение от пользователя из консоли
